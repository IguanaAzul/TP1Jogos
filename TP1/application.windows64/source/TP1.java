...
public class TP1 extends PApplet {
...
public void posiciona(){
    ...
    if(!PAUSE){
      ...
      if(es>enemyShootDelay) {enemyShooting=true; es=0; if(vxBI>0) vxBI*=-1.01f; else vxBI*=-1;}
    }
...
public void desenha(){
  ...
  if(VITORIA){
    ...
    if(JUST){
      coins+=100;
      vxInicialTiroInimigo-=0.5f;
      if(vyInicialTiroInimigo>8) vyInicialTiroInimigo+=0.5f;
      xB=10; vxB=0;
      vida=vidaMax;
      JUST=false;
      vidaInimigoMax+=1;
...
class Drop {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "TP1" };
    if (passedArgs != null) {
